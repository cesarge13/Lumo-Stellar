...
export default function PassengerProfile() {
  ...
  useEffect(() => {
    ...
    // Cargar perfil específico del rol si existe
    loadRoleProfile()
  }, [user, navigate])

  const loadRoleProfile = async () => {
    try {
      // TODO: Implementar endpoint para obtener perfil por rol
      // Por ahora usamos datos del usuario
      if (user) {
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Por ahora actualizamos el perfil general
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          {t('profile.passenger.description') || 'Gestiona tu perfil de pasajero'}
        </p>
      </div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t('profile.avatarDescription') || 'Tu foto de perfil'}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t('profile.displayNameDescription') || 'Este nombre será visible para los conductores'}
                </p>
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder={t('profile.bioPlaceholder') || 'Cuéntanos sobre ti...'}
                  rows={4}
                />
                <p className="text-xs text-muted-foreground mt-1">
                  {t('profile.bioDescription') || 'Información adicional sobre ti (opcional)'}
                </p>
              </div>
            </CardContent>
          </Card>
